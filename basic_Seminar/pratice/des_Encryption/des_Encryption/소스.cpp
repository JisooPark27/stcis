...
void ip() {
	...
	printf("Plaintext 입력 : ");
	for (int i = 0; i < 16; i++) {
		scanf("%1x", &plainTextIn[i]);
	}


	//평문 64비트 전환
	for (int i = 0; i < 16; i++) {
		beforeIp_64[4 * i + 3] = plainTextIn[i] % 2;
		beforeIp_64[4 * i + 2] = plainTextIn[i] / 2 % 2;
		beforeIp_64[4 * i + 1] = plainTextIn[i] / 4 % 2;
		beforeIp_64[4 * i] = plainTextIn[i] / 8 % 2;
	}
...
//키
void key_function() {
	for (int i = 0; i<shiftNum[round]; i++) {
		shift();
	}
